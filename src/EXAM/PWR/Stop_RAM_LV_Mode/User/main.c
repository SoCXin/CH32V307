...
/*
 *@Note
 停止模式开启LV时，RAM 2K、30K低功耗数据保持例程：
 本例程演示在 2K RAM和30K RAM指定位置处写入数据，之后WFI进入STOP睡眠模式并开启LV，通过PA2引脚
...
void TestDataWrite(void)
{
    ...
    {
        *(uint32_t volatile *)(myAddr+(i<<2)) = 0x22222222;
    }
    for(i=0;i<1024;i++)//Check 30K RAM
    {
        if((*(uint32_t volatile *)(myAddr+(i<<2))) != 0x22222222)
        {
    ...
    {
        *(uint32_t volatile *)(myAddr2+(i<<2)) = 0x22222222;
    }
    for(i=0;i<10;i++)//Check 30K RAM
    {
        if((*(uint32_t volatile *)(myAddr2+(i<<2))) != 0x22222222)
        {
...
int main(void)
{
    ...
    {
        TestDataRead();
        printf("2.Stop RAM LV Mode Test\r\n");
        TestDataWrite();
